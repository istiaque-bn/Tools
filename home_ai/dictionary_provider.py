...
def normalise_payload(payload, fallback_word):
    if not isinstance(payload, list) or not payload:
        raise DictionaryLookupError(
            f'No dictionary entry was found for "{fallback_word}".'
        )
    result = {
        "word": payload[0].get("word") or fallback_word,
        "phonetic": "",
        "pronunciations": [],
        "meanings": [],
        "source_urls": [],
    ...
    for entry in payload:
        ...
        result["source_urls"].extend(
            url
            for url in entry.get("sourceUrls", [])
            if url not in result["source_urls"]
        )
        for phonetic in entry.get("phonetics", []):
            ...
            result["pronunciations"].append(
                {"text": text, "audio": audio, "region": _region(audio)}
            )
        for meaning in entry.get("meanings", []):
            definitions, synonyms, antonyms = (
                [],
                list(meaning.get("synonyms") or []),
                list(meaning.get("antonyms") or []),
            )
...
def _fetch_online(word, timeout=4):
    request = Request(
        API_ROOT + quote(word),
        headers={"Accept": "application/json", "User-Agent": "NecessaryTools/1.0"},
    )
...
def lookup_word(word):
    ...
    if not WORD_PATTERN.fullmatch(word):
        raise DictionaryLookupError(
            "Enter an English word or short phrase using letters, spaces, apostrophes, or hyphens."
        )
    ...
    try:
        ...
        if exc.code == 404:
            raise DictionaryLookupError(
                f'No offline entry was found for "{word}", and no online entry is available.'
            ) from exc
        raise DictionaryLookupError(
            f'No offline entry was found for "{word}". Connect to the internet and try again.'
        ) from exc
