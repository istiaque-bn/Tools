...
def image_upload(name="sample.png", size=(120, 80), image_format="PNG"):
    ...
    return SimpleUploadedFile(
        name, output.getvalue(), content_type=f"image/{image_format.lower()}"
    )
...
class PdfToolkitTests(TestCase):
    ...
    def test_tool_requires_login(self):
        ...
        self.assertRedirects(
            response,
            reverse("login") + "?next=" + reverse("pdf_toolkit"),
    ...
    def test_split_returns_requested_range(self):
        ...
        response = self.client.post(
            ...
            {
                "action": "split",
                "pdf_file": pdf_upload("source.pdf", 4),
                "start_page": "2",
                "end_page": "3",
            },
        )
    ...
    def test_split_multiple_ranges_returns_zip_of_pdfs(self):
        ...
        response = self.client.post(
            ...
            {
                "action": "split",
                "pdf_file": pdf_upload("source.pdf", 8),
        ...
        page_counts = [
            len(PdfReader(BytesIO(archive.read(name))).pages)
    ...
    def test_split_multiple_ranges_rejects_out_of_bounds_pages(self):
        ...
        self.assertContains(
            response, "Every range must be between 1 and 4.", status_code=400
        )
    ...
    def test_inspect_displays_details(self):
        ...
        response = self.client.post(
            ...
            {
                "action": "inspect",
                "pdf_file": pdf_upload("details.pdf", 2),
            },
        )
    ...
    def test_remove_and_reorder_pages(self):
        ...
        self.assertEqual(
            len(PdfReader(BytesIO(b"".join(removed.streaming_content))).pages), 2
        )
        organized = self.client.post(
            reverse("pdf_toolkit"),
            {
                "action": "organize",
                "pdf_file": pdf_upload("source.pdf", 3),
                "pages": "3,1,2,1",
            },
        )
        self.assertEqual(
            len(PdfReader(BytesIO(b"".join(organized.streaming_content))).pages), 4
        )
    ...
    def test_watermark_and_page_numbers(self):
        ...
        for action in ("watermark", "number"):
            ...
            self.assertEqual(response.status_code, 200)
            self.assertEqual(
                len(PdfReader(BytesIO(b"".join(response.streaming_content))).pages), 2
            )
    ...
    def test_protect_and_unlock(self):
        ...
        protected = self.client.post(
            ...
            {
                "action": "protect",
                "pdf_file": pdf_upload("source.pdf"),
        ...
        unlocked = self.client.post(
            reverse("pdf_toolkit"),
            {
                ...
                "pdf_file": SimpleUploadedFile(
                    "locked.pdf", encrypted_bytes, content_type="application/pdf"
                ),
    ...
    def test_preview_returns_page_thumbnails(self):
        self.client.force_login(self.user)
        response = self.client.post(
            reverse("pdf_preview"), {"pdf_file": pdf_upload("preview.pdf", 2)}
        )
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(len(payload["pages"]), 2)
        self.assertTrue(
            payload["pages"][0]["thumbnail"].startswith("data:image/jpeg;base64,")
        )
    ...
    def test_docx_to_pdf_returns_download(self, converter):
        ...
        response = self.client.post(
            reverse("pdf_toolkit"), {"action": "docx_to_pdf", "docx_files": [upload]}
        )
        ...
        self.assertEqual(
            len(PdfReader(BytesIO(b"".join(response.streaming_content))).pages), 1
        )
    ...
    def test_multiple_docx_files_return_zip_of_pdfs(self, converter):
        ...
        response = self.client.post(
            reverse("pdf_toolkit"), {"action": "docx_to_pdf", "docx_files": uploads}
        )
...
class ImageToolkitTests(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            "image-tester", password="test-password"
        )
        self.client.force_login(self.user)
    ...
    def test_compress_and_convert(self):
        compressed = self.image_response(
            {"action": "compress_image", "image_file": image_upload(), "quality": "65"}
        )
        self.assertEqual(compressed.size, (120, 80))
        converted = self.image_response(
            {
    ...
    def test_crop_returns_only_selected_region(self):
        cropped = self.image_response(
            {
                "action": "crop_image",
                "image_file": image_upload(size=(120, 80)),
                "crop_x": "20",
                "crop_y": "10",
                "crop_width": "50",
                "crop_height": "30",
    ...
    def test_inspect_displays_image_details(self):
        response = self.client.post(
            reverse("image_toolkit"),
            {"action": "inspect_image", "image_file": image_upload()},
        )
        ...
        watermarked = self.image_response(
            {
                ...
                "image_file": image_upload(size=(300, 200)),
                "watermark_text": "TEST",
                "watermark_size": "8",
...
class ExpandedToolkitTests(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            "tools-user", password="test-password"
        )
        self.client.force_login(self.user)
    ...
    def test_qr_generation_returns_png(self):
        response = self.client.post(
            reverse("qr_toolkit"),
            {"action": "generate", "kind": "text", "value": "https://example.com"},
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Type"], "image/png")

    def test_qr_scan_reads_uploaded_code(self):
        output = BytesIO()
        qrcode.make("offline QR scan").save(output, "PNG")
        upload = SimpleUploadedFile(
            "code.png", output.getvalue(), content_type="image/png"
        )
        response = self.client.post(
            reverse("qr_toolkit"), {"action": "scan", "image_file": upload}
        )
        self.assertContains(response, "offline QR scan")

    def test_text_tools_and_api(self):
        response = self.client.post(
            reverse("text_toolkit"), {"action": "json", "text": '{"ready":true}'}
        )
        self.assertContains(response, "&quot;ready&quot;: true")
        api = self.client.post(
            reverse("tools_api"),
            json.dumps({"action": "slug", "text": "Hello New World"}),
    ...
    def test_batch_images_returns_zip(self):
        response = self.client.post(
            reverse("batch_images"),
            {
                "action": "compress",
                "format": "PNG",
                "image_files": [image_upload("one.png"), image_upload("two.png")],
            },
        )
        self.assertEqual(response["Content-Type"], "application/zip")
        archive = zipfile.ZipFile(BytesIO(b"".join(response.streaming_content)))
    ...
    def test_archive_create_7z(self):
        file = SimpleUploadedFile("note.txt", b"hello", content_type="text/plain")
        created = self.client.post(
            reverse("archive_toolkit"),
            {
        ...
        self.assertEqual(created["Content-Type"], "application/x-7z-compressed")
        self.assertTrue(py7zr.is_7zfile(BytesIO(b"".join(created.streaming_content))))

    def test_archive_password_create_and_remove(self):
        file = SimpleUploadedFile("secret.txt", b"private", content_type="text/plain")
        protected = self.client.post(
            reverse("archive_toolkit"),
            {
                "action": "create",
        ...
        protected_data = b"".join(protected.streaming_content)
        unlocked = self.client.post(
            reverse("archive_toolkit"),
            {
                "action": "remove_password",
                "current_password": "strong pass",
                "archive": SimpleUploadedFile("protected.zip", protected_data),
            },
        )
        with zipfile.ZipFile(BytesIO(b"".join(unlocked.streaming_content))) as archive:
            self.assertEqual(archive.read("secret.txt"), b"private")

    def test_metadata_cleaner_removes_pdf_properties(self):
        source = BytesIO()
        writer = PdfWriter()
        writer.add_blank_page(width=612, height=792)
        writer.add_metadata({"/Author": "Private Author", "/Title": "Private Title"})
        writer.write(source)
        upload = SimpleUploadedFile(
            "private.pdf", source.getvalue(), content_type="application/pdf"
        )

        response = self.client.post(
            reverse("advanced_toolkit"), {"action": "clean_metadata", "file": upload}
        )

        self.assertEqual(response.status_code, 200)
        cleaned = PdfReader(BytesIO(b"".join(response.streaming_content)))
        self.assertNotEqual((cleaned.metadata or {}).get("/Author"), "Private Author")

    def test_metadata_cleaner_removes_office_properties_and_comments(self):
        source = BytesIO()
        with zipfile.ZipFile(source, "w") as archive:
            archive.writestr(
                "[Content_Types].xml",
                '<?xml version="1.0"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Override PartName="/word/document.xml" ContentType="application/xml"/><Override PartName="/word/comments.xml" ContentType="application/xml"/></Types>',
            )
            archive.writestr(
                "word/document.xml",
                '<?xml version="1.0"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>Visible text</w:t></w:r><w:commentReference w:id="0"/></w:p></w:body></w:document>',
            )
            archive.writestr(
                "word/comments.xml", "<comments>private comment</comments>"
            )
            archive.writestr("docProps/core.xml", "<core>Private Author</core>")
        upload = SimpleUploadedFile(
            "private.docx",
            source.getvalue(),
            content_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        ...
        self.assertEqual(response.status_code, 200)
        cleaned = zipfile.ZipFile(BytesIO(b"".join(response.streaming_content)))
        self.assertNotIn("word/comments.xml", cleaned.namelist())
        self.assertNotIn("docProps/core.xml", cleaned.namelist())
        self.assertIn(b"Visible text", cleaned.read("word/document.xml"))
