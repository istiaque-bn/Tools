...
def _download(writer, filename):
    ...
    output.seek(0)
    return FileResponse(
        output, as_attachment=True, filename=filename, content_type="application/pdf"
    )
...
def _image_download(image, output_format, filename, quality=88):
    ...
    if output_format == "JPEG" and image.mode not in {"RGB", "L"}:
        ...
        background.paste(
            image, mask=image.getchannel("A") if "A" in image.getbands() else None
        )
        image = background
    output = BytesIO()
    ...
    return FileResponse(
        ...
        content_type=f"image/{'jpeg' if output_format == 'JPEG' else extension}",
    )
...
def login_view(request):
    if request.user.is_authenticated:
        return redirect(
            "admin_panel:dashboard" if is_admin_user(request.user) else "home"
        )
...
def home_view(request):
    ...
    return render(
        request,
        "home.html",
        {"docx_abbreviation_tool_enabled": settings.DOCX_ABBREVIATION_TOOL_ENABLED},
    )
    ...
    try:
        ...
        if action == "split":
            ...
            if start < 1 or end < start or end > len(reader.pages):
                raise ValueError(
                    f"Enter a page range between 1 and {len(reader.pages)}."
                )
            writer = PdfWriter()
            for index in range(start - 1, end):
                writer.add_page(reader.pages[index])
            ...
            for index, page in enumerate(reader.pages, start=1):
                ...
                page.merge_page(
                    _overlay_page(
                        ...
                        height,
                        text,
                        page_number=index if action == "number" else None,
                    )
                )
                writer.add_page(page)
            return _download(
                writer, "numbered.pdf" if action == "number" else "watermarked.pdf"
            )
        ...
        if action == "image_to_pdf":
            ...
            images = []
            for upload in uploads:
                if upload.size > MAX_IMAGE_SIZE or not upload.name.lower().endswith(
                    (".jpg", ".jpeg", ".png")
                ):
        ...
        if action == "docx_to_pdf":
            ...
            return FileResponse(
                archive,
                as_attachment=True,
                filename="converted-docx-files.zip",
                content_type="application/zip",
            )
        ...
        context["active_tool"] = (
            action if action in {"merge", "split", "inspect"} else "merge"
        )
...
def pdf_preview_view(request):
    ...
    try:
        ...
        if document.needs_pass:
            return JsonResponse(
                {"error": "Password-protected PDFs cannot be previewed yet."},
                status=400,
            )
        pages = []
            ...
            pages.append(
                {
                    ...
                    + base64.b64encode(pixmap.tobytes("jpeg", jpg_quality=70)).decode(
                        "ascii"
                    ),
        ...
        return JsonResponse(
            {"error": str(exc) or "This PDF could not be previewed."}, status=400
        )
    ...
    try:
        ...
        if action == "compress_image":
            return _image_download(
                image, safe_format, "compressed", request.POST.get("quality", 75)
            )

        if action == "resize_image":
            width, height = (
                int(request.POST.get("width", 0) or 0),
                int(request.POST.get("height", 0) or 0),
            )
            ...
            if width < 1 or height < 1 or width * height > 200_000_000:
                raise ValueError(
                    "Resulting dimensions must be valid and below 200 megapixels."
                )
            return _image_download(
        ...
        if action == "crop_image":
            left, top = (
                int(request.POST.get("crop_x", 0)),
                int(request.POST.get("crop_y", 0)),
            )
            width, height = (
                int(request.POST.get("crop_width", 0)),
                int(request.POST.get("crop_height", 0)),
            )
            if (
                width < 1
                or height < 1
        ...
        if action == "transform_image":
            ...
            return _image_download(
                operations[transform](image), safe_format, "transformed"
            )

        if action == "convert_image":
            return _image_download(
                image,
                request.POST.get("output_format", "PNG"),
                "converted",
                request.POST.get("quality", 88),
            )
        ...
        if action == "watermark_image":
            ...
            font_size = max(
                14,
                round(
                    min(image.size) * int(request.POST.get("watermark_size", 5)) / 100
                ),
            )
            try:
            ...
            position = (
                max(
                    0,
                    min(
                        working.width - (box[2] - box[0]),
                        int(request.POST.get("watermark_x", default_x)),
                    ),
                ),
                max(
                    0,
                    min(
                        working.height - (box[3] - box[1]),
                        int(request.POST.get("watermark_y", default_y)),
                    ),
                ),
            )
            ...
            draw.text(
                ...
                stroke_fill=(0, 0, 0, round(150 * opacity / 100)),
            )
            return _image_download(
                Image.alpha_composite(working, layer), safe_format, "watermarked"
            )

        if action == "adjust_image":
            result = image.convert("RGB")
            result = ImageEnhance.Brightness(result).enhance(
                float(request.POST.get("brightness", 100)) / 100
            )
            result = ImageEnhance.Contrast(result).enhance(
                float(request.POST.get("contrast", 100)) / 100
            )
            result = ImageEnhance.Color(result).enhance(
                float(request.POST.get("saturation", 100)) / 100
            )
            result = ImageEnhance.Sharpness(result).enhance(
                float(request.POST.get("sharpness", 100)) / 100
            )
        ...
        if action == "image_base64":
            ...
            response = HttpResponse(
                f"data:{Image.MIME.get(source_format, 'application/octet-stream')};base64,{encoded}",
        ...
        return render(
            request,
            "image_toolkit.html",
            {"image_error": str(exc), "active_image_tool": action},
            status=400,
        )
