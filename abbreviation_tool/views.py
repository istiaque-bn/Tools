...
from .forms import (
    AbbreviationEntryForm,
    DictionaryImportForm,
    DictionarySearchForm,
    DocumentUploadForm,
    FeedbackForm,
    PowerPointProcessForm,
    QuickProcessForm,
)
from .models import (
...
def landing(request):
    ...
    if request.method == "POST" and form.is_valid():
        active_sessions = DocumentProcessingSession.objects.filter(
            user=request.user, deleted_at__isnull=True, expires_at__gt=timezone.now()
        ).count()
        if active_sessions >= settings.DOCX_ABBREVIATION_MAX_ACTIVE_SESSIONS:
            return render(
                request,
                "abbreviation_tool/landing.html",
                {
                    "form": form,
                    "powerpoint_form": PowerPointProcessForm(),
                    "error": "Please wait for an existing document session to expire or cancel it.",
                },
                status=429,
            )
        document = form.cleaned_data["docx_file"]
        profile = (
            AbbreviationProfile.objects.filter(name="General", active=True).first()
            or AbbreviationProfile.objects.filter(active=True).first()
        )
        session = DocumentProcessingSession.objects.create(
            ...
            + timedelta(minutes=settings.DOCX_ABBREVIATION_SESSION_TTL_MINUTES),
        )
        try:
            save_original(session, document)
            analyse_session(
                session, policy=session.replacement_policy, include_tables=True
            )
            session.suggestions.filter(ambiguity_status="unambiguous").update(
                review_status="accepted"
            )
            session.suggestions.filter(ambiguity_status="ambiguous").update(
                review_status="rejected"
            )
            session.accepted_count = session.suggestions.filter(
                review_status="accepted"
            ).count()
            session.rejected_count = session.suggestions.filter(
                review_status="rejected"
            ).count()
            ...
            return render(
                ...
                "abbreviation_tool/landing.html",
                {
                    "form": form,
                    "powerpoint_form": PowerPointProcessForm(),
                    "error": "The document could not be processed. Temporary files were deleted.",
                },
                status=400,
            )
    return render(
        ...
        "abbreviation_tool/landing.html",
        {
            "form": form,
            "powerpoint_form": PowerPointProcessForm(),
            "entry_count": AbbreviationEntry.objects.filter(
...
def powerpoint_convert(request):
    ...
    try:
        ...
        output, count = process_powerpoint(
            form.cleaned_data["presentation_file"],
            form.cleaned_data["operation_type"],
            profile,
        )
    except ValidationError as exc:
    ...
    response = FileResponse(
        output,
        as_attachment=True,
        filename=f"processed-{source_name}.pptx",
...
def text_convert(request):
    ...
    if operation not in {
        DocumentProcessingSession.Operation.ABBREVIATE,
        DocumentProcessingSession.Operation.DEABBREVIATE,
    }:
        return JsonResponse({"error": "Choose abbreviate or deabbreviate."}, status=400)
    if not text.strip() or len(text) > 50_000:
        return JsonResponse(
            {"error": "Enter between 1 and 50,000 characters."}, status=400
        )
    ...
    container = TextContainer(
        ...
        "paragraph",
        text,
        [CharacterLocation(0, index, b"") for index in range(len(text))],
    )
    matches = find_matches(
        container, candidates_for(profile, operation), operation, POLICY_ALL
    )
...
def dictionary(request):
    ...
    entries = AbbreviationEntry.objects.select_related("category").prefetch_related(
        "profiles"
    )
    if form.is_valid():
        ...
        if query:
            entries = entries.filter(
                ...
                | models.Q(full_form__icontains=query)
            )
    ...
    return render(
        request,
        "abbreviation_tool/dictionary.html",
        {"form": form, "entries": entries[:250]},
    )
...
def manage_dictionary(request, entry_id=None):
    ...
    import_form = DictionaryImportForm(
        request.POST or None, request.FILES or None, prefix="import"
    )
    if request.method == "POST" and "save_entry" in request.POST and form.is_valid():
        before = (
            None
            if entry is None
            else {"abbreviation": entry.abbreviation, "full_form": entry.full_form}
        )
        ...
        try:
            ...
            general_profile = AbbreviationProfile.objects.filter(
                name="General", active=True
            ).first()
            if general_profile:
                saved.profiles.add(general_profile)
            AbbreviationAuditLog.objects.create(
                abbreviation_entry=saved,
                action="updated" if entry else "created",
                previous_value=before,
                new_value={
                    "abbreviation": saved.abbreviation,
                    "full_form": saved.full_form,
                },
                user=request.user,
            )
            messages.success(
                request, "Abbreviation updated." if entry else "Abbreviation added."
            )
            return redirect("abbreviation_tool:manage_dictionary")
    if (
        request.method == "POST"
        and "import_entries" in request.POST
        and import_form.is_valid()
    ):
        ...
        messages.success(
            request,
            f"Import complete: {result['created']} added, {result['updated']} updated, {result['skipped']} skipped.",
        )
        return redirect("abbreviation_tool:manage_dictionary")
    ...
    meaning_counts = (
        AbbreviationEntry.objects.filter(
            normalized_abbreviation=OuterRef("normalized_abbreviation")
        )
        .values("normalized_abbreviation")
        .annotate(total=Count("normalized_full_form", distinct=True))
        .values("total")
    )
    managed_entries = AbbreviationEntry.objects.annotate(
        meaning_count=Subquery(meaning_counts, output_field=IntegerField())
    )
    all_entries = managed_entries.order_by("full_form", "abbreviation")[:500]
    search_results = AbbreviationEntry.objects.none()
    if query:
        search_results = managed_entries.filter(
            models.Q(abbreviation__icontains=query)
            | models.Q(full_form__icontains=query)
        ).order_by("full_form", "abbreviation")[:500]
    return render(
        request,
        "abbreviation_tool/manage_dictionary.html",
        {
            "form": form,
...
def export_dictionary_xlsx(request):
    ...
    for abbreviation, full_form in (
        AbbreviationEntry.objects.order_by("abbreviation", "full_form")
        .values_list("abbreviation", "full_form")
        .iterator()
    ):
        worksheet.append((abbreviation, full_form))
...
def _import_dictionary_file(upload, user):
    ...
    if upload.name.lower().endswith(".xlsx"):
        ...
        try:
            ...
            if not first_row:
                ...
                return result
            fieldnames = [
                str(value).strip() if value is not None else "" for value in first_row
            ]
            reader = (
                {
        ...
        try:
            reader = csv.DictReader(
                io.StringIO(upload.read().decode("utf-8-sig"), newline="")
            )
    ...
    short_key = (
        headers.get("abbreviation")
        or headers.get("short form")
        or headers.get("short_form")
    )
    ...
    for line, row in enumerate(reader, 2):
        ...
        if " / " in full_form:
            ...
            result["errors"].append(
                f"Row {line}: use a separate row for each full form instead of '/'."
            )
            continue
        if (
            not abbreviation
            or not full_form
            or len(abbreviation) > 100
            or len(full_form) > 500
        ):
            result["skipped"] += 1
        ...
        existing = (
            AbbreviationEntry.objects.filter(
                normalized_full_form=AbbreviationEntry.normalize(full_form)
            )
            .order_by("pk")
            .first()
        )
        if existing and existing.normalized_abbreviation == AbbreviationEntry.normalize(
            abbreviation
        ):
            result["skipped"] += 1
            continue
        before = (
            {"abbreviation": existing.abbreviation, "full_form": existing.full_form}
            if existing
            else None
        )
        entry = existing or AbbreviationEntry(created_by=user)
        entry.abbreviation, entry.full_form, entry.updated_by = (
            abbreviation,
            full_form,
            user,
        )
        ...
        AbbreviationAuditLog.objects.create(
            abbreviation_entry=entry,
            action="import_updated" if existing else "import_created",
            previous_value=before,
            new_value={"abbreviation": abbreviation, "full_form": full_form},
            user=user,
        )
        general_profile = AbbreviationProfile.objects.filter(
            name="General", active=True
...
def feedback(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        item.save()
        messages.success(
            request, "Thank you. Your feedback was sent to the administrator."
        )
...
def upload(request):
    ...
    if (
        request.method == "POST"
        and active_sessions >= settings.DOCX_ABBREVIATION_MAX_ACTIVE_SESSIONS
    ):
        return render(
            request,
            "abbreviation_tool/upload.html",
            {
                "form": DocumentUploadForm(),
                "limit_error": "Finish or cancel an existing document session before starting another.",
            },
            status=429,
        )
    ...
    if request.method == "POST" and form.is_valid():
        ...
        session = DocumentProcessingSession.objects.create(
            ...
            processing_options={
                ...
                "include_footnotes_endnotes": form.cleaned_data[
...
def session_detail(request, session_id):
    ...
    return render(
        request, "abbreviation_tool/session.html", {"processing_session": session}
    )
...
def analyse(request, session_id):
    ...
    try:
        analyse_session(
            session,
            policy=session.replacement_policy,
            include_tables=session.processing_options.get("include_tables", True),
        )
        ...
        expire_session(session, status=DocumentProcessingSession.Status.FAILED)
        messages.error(
            request, "Document analysis failed and its temporary files were deleted."
        )
        return redirect("abbreviation_tool:landing")
...
def _owned_review_session(user, session_id):
    session = get_object_or_404(
        DocumentProcessingSession.objects.prefetch_related(
            "suggestions__abbreviation_entry__category"
        ),
        id=session_id,
        user=user,
        deleted_at__isnull=True,
...
def review(request, session_id):
    ...
    suggestions = session.suggestions.select_related(
        "abbreviation_entry__category", "selected_meaning"
    ).order_by("container_identifier", "start_offset")
    ambiguity_options = {}
    for suggestion in suggestions.filter(ambiguity_status="ambiguous"):
        ambiguity_options[str(suggestion.id)] = list(
            AbbreviationEntry.objects.filter(
    ...
    return render(
        request,
        "abbreviation_tool/review.html",
        {
            "processing_session": session,
            "suggestions": suggestions,
            "preview": build_preview(session),
            "ambiguity_options": ambiguity_options,
            "categories": suggestions.exclude(abbreviation_entry__category__isnull=True)
            .values_list("abbreviation_entry__category__name", flat=True)
            .distinct(),
...
def suggestion_api(request, session_id, suggestion_id):
    ...
    try:
        suggestion = decide(
            ...
            data.get("selected_meaning_id"),
        )
        return JsonResponse(
            {
                "ok": True,
                "status": suggestion.review_status,
                "replacement": suggestion.user_modified_text
                or suggestion.proposed_text,
            }
        )
        ...
        message = (
            ...
            else "The selected meaning is invalid."
        )
...
def bulk_review_api(request, session_id):
    ...
    try:
        bulk_decide(
            ...
            request.user,
            data.get("action"),
            bool(data.get("high_confidence")),
            data.get("suggestion_ids"),
        )
...
def generate(request, session_id):
    ...
    try:
        ...
        logger.exception(
            ...
            extra={
                "processing_session_id": str(session_id),
                "user_id": request.user.id,
            },
        )
        session = get_object_or_404(
            DocumentProcessingSession, id=session_id, user=request.user
        )
        expire_session(session, status=DocumentProcessingSession.Status.FAILED)
        messages.error(
            request, "DOCX generation failed and its temporary files were deleted."
        )
        return redirect("abbreviation_tool:landing")
...
def summary(request, session_id):
    ...
    session = get_object_or_404(
        ...
        id=session_id,
        user=request.user,
        status=DocumentProcessingSession.Status.COMPLETE,
        deleted_at__isnull=True,
    )
    return render(
        request, "abbreviation_tool/summary.html", {"processing_session": session}
    )
...
def glossary_download(request, session_id):
    ...
    content = "Abbreviation\tFull Form\n" + "\n".join(
        f"{abbreviation}\t{full_form}" for abbreviation, full_form in rows
    )
    response = HttpResponse(
        content, content_type="text/tab-separated-values; charset=utf-8"
    )
