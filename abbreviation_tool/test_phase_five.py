...
def complex_docx():
    ...
    with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as package:
        ...
        package.writestr("word/header1.xml", b"unchanged-header")
        package.writestr("word/footer1.xml", b"unchanged-footer")
        package.writestr("word/media/image1.png", b"unchanged-image")
    from django.core.files.uploadedfile import SimpleUploadedFile

    return SimpleUploadedFile(
        "formatted.docx", output.getvalue(), content_type=DOCX_MIME
    )
...
class GenerationTests(TestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user("generation-user")
        ...
        ProcessingSuggestion.objects.create(
            session=self.session,
            abbreviation_entry=self.baf,
            operation_type="abbreviate",
            original_text="Bangladesh Armed Forces",
            proposed_text="BAF",
            ...
            mixed_format_warning=True,
        )
        ProcessingSuggestion.objects.create(
            session=self.session,
            abbreviation_entry=self.hq,
            operation_type="abbreviate",
            original_text="Headquarters",
            proposed_text="HQ",
            user_modified_text="Main HQ",
            container_type="paragraph",
            container_identifier="word/document.xml:p0",
            paragraph_identifier="word/document.xml:p0",
            start_offset=28,
    ...
    def test_generation_preserves_formatting_and_unchanged_parts(self):
        ...
        with zipfile.ZipFile(original) as package:
            hashes = {
                name: hashlib.sha256(package.read(name)).digest()
                for name in (
                    "word/header1.xml",
                    "word/footer1.xml",
        ...
        with zipfile.ZipFile(output) as package:
            ...
            self.assertIn("BAF and Main HQ.", "".join(root.itertext()))
            self.assertEqual(
                root.find(".//{*}pgSz").get(
            ...
            self.assertIsNotNone(root.find(".//{*}pgMar"))
            self.assertIsNotNone(root.find(".//{*}pStyle"))
            self.assertIsNotNone(root.find(".//{*}b"))
            for name, digest in hashes.items():
                self.assertEqual(hashlib.sha256(package.read(name)).digest(), digest)
    ...
    def test_cross_user_cannot_generate(self):
        with self.assertRaises(DocumentProcessingSession.DoesNotExist):
            generate_session(self.session.id, self.other)

    def test_one_time_download_cleans_after_close(self):
        generate_session(self.session.id, self.user)
        self.client.force_login(self.user)
        response = self.client.get(
            reverse("abbreviation_tool:download", args=[self.session.id])
        )
        ...
        self.assertFalse(session_directory(self.session.id).exists())
        self.assertEqual(
            self.client.get(
                reverse("abbreviation_tool:download", args=[self.session.id])
            ).status_code,
    ...
    def test_pending_ambiguity_blocks_generation(self):
        item = self.session.suggestions.first()
        item.ambiguity_status = "ambiguous"
        item.review_status = "pending"
        item.save(update_fields=("ambiguity_status", "review_status"))
