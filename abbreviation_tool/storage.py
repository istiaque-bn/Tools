...
def cleanup_expired(now=None):
    ...
    sessions = DocumentProcessingSession.objects.filter(
    ...
    for session in sessions.iterator():
        ...
        count += 1
    cutoff = (
        now - timedelta(minutes=settings.DOCX_ABBREVIATION_SESSION_TTL_MINUTES)
    ).timestamp()
    active_ids = {
        str(value)
        for value in DocumentProcessingSession.objects.filter(
