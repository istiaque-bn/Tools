...
def advanced_docx():
    ...
    with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as package:
        package.writestr("[Content_Types].xml", CONTENT_TYPES); package.writestr("_rels/.rels", RELS)
        package.writestr("word/document.xml", BODY); package.writestr("word/header1.xml", HEADER); package.writestr("word/footer1.xml", FOOTER)
        package.writestr("word/footnotes.xml", FOOTNOTES); package.writestr("word/endnotes.xml", ENDNOTES)
    return SimpleUploadedFile("advanced.docx", output.getvalue(), content_type=DOCX_MIME)
...
class AdvancedStructureTests(TestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user("advanced-user"); self.profile = AbbreviationProfile.objects.get(name="General")
        for abbreviation, full_form in (("HQ", "Headquarters"), ("CO", "Commanding Officer")):
            entry = AbbreviationEntry.objects.create(abbreviation=abbreviation, full_form=full_form); entry.profiles.add(self.profile)

    def tearDown(self): self.override.disable(); self.temp.cleanup()

    def session(self, options):
        upload = advanced_docx()
        session = DocumentProcessingSession.objects.create(user=self.user, original_filename="advanced.docx", operation_type="abbreviate", profile=self.profile, file_size=upload.size, processing_options=options, expires_at=timezone.now()+timedelta(minutes=30))
        save_original(session, upload); return session

    def test_analysis_includes_selected_advanced_parts(self):
        session = self.session({"include_tables": True, "include_headers_footers": True, "include_footnotes_endnotes": True})
        analyse_session(session, POLICY_ALL)
        types = set(session.suggestions.values_list("container_type", flat=True))
        self.assertTrue({"paragraph", "header", "footer", "footnote", "endnote"}.issubset(types))
        self.assertTrue(session.suggestions.filter(container_identifier__startswith="word/document.xml").exists())

    def test_generation_modifies_header_and_preserves_other_parts(self):
        session = self.session({"include_headers_footers": True, "include_footnotes_endnotes": False, "glossary_mode": "none"})
        analyse_session(session, POLICY_ALL)
        session.suggestions.filter(container_type="header").update(review_status="accepted")
        session.suggestions.exclude(container_type="header").update(review_status="rejected")
    ...
    def test_glossary_insertion_before_section_and_at_bookmark(self):
        inserted = insert_glossary(BODY, [("CO", "Commanding Officer"), ("HQ", "Headquarters")])
        root = etree.fromstring(inserted); body = root.find(f"{{{WNS}}}body")
        self.assertEqual(body[-1].tag, f"{{{WNS}}}sectPr"); self.assertEqual(len(root.findall(".//{*}tbl")), 1)
