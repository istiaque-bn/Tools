...
FORBIDDEN_SUFFIXES = {".exe", ".dll", ".com", ".bat", ".cmd", ".js", ".jar", ".ps1", ".sh"}
    ...
    try:
        ...
        with zipfile.ZipFile(upload) as package:
            ...
            if total > settings.DOCX_ABBREVIATION_MAX_UNCOMPRESSED_MB * 1024 * 1024:
                raise ValidationError("The expanded DOCX is too large to process safely.")
            for info in infos:
                if not _safe_member(info.filename):
                    raise ValidationError("The DOCX contains an unsafe package path.")
                suffix = PurePosixPath(info.filename).suffix.lower()
                if suffix in FORBIDDEN_SUFFIXES:
                    raise ValidationError("The DOCX contains a forbidden embedded file.")
                if info.compress_size and info.file_size / info.compress_size > settings.DOCX_ABBREVIATION_MAX_ZIP_RATIO:
                    raise ValidationError("The DOCX contains a suspicious compression ratio.")
                lower_name = info.filename.lower()
                if "vbaproject" in lower_name or lower_name.endswith(".bin"):
                    raise ValidationError("Macro or executable OOXML content is not supported.")
                if info.flag_bits & 0x1:
                    raise ValidationError("Encrypted or password-protected DOCX files are not supported.")
                if suffix in XML_SUFFIXES and info.file_size:
                    content = package.read(info, pwd=None)
                    prefix = content[:2048].upper()
                    if b"<!DOCTYPE" in prefix or b"<!ENTITY" in prefix:
                        raise ValidationError("Unsafe XML declarations are not supported.")
                    if suffix == ".rels" and re.search(br'TargetMode\s*=\s*["\']External["\']', content, re.I):
                        raise ValidationError("Documents with external relationships are not supported.")
            content_types = package.read("[Content_Types].xml")
            if b"macroEnabled" in content_types or b"vbaProject" in content_types:
                raise ValidationError("Macro-enabled documents are not supported.")
            unsupported = tuple(name for name in names if name.startswith(("word/embeddings/", "word/diagrams/")))
