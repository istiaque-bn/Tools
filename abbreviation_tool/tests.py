...
class AbbreviationToolPhaseOneTests(TestCase):
    ...
    def setUpTestData(cls):
        AbbreviationEntry.objects.create(
            abbreviation="SITREP",
            full_form="Situation Report",
            source_name="JSSDM 2022",
        )
        AbbreviationEntry.objects.create(
            abbreviation="AA", full_form="Avenue of Approach", is_ambiguous=True
        )
        AbbreviationEntry.objects.create(
            abbreviation="AA", full_form="Anti-Aircraft", is_ambiguous=True
        )

    def setUp(self):
        self.user = get_user_model().objects.create_user(
            "docx-user", password="test-password"
        )
    ...
    def test_all_authenticated_users_receive_checker_permission(self):
        ...
        self.assertEqual(
            self.client.get(reverse("abbreviation_tool:landing")).status_code, 200
        )
    ...
    def test_feature_flag_disables_route(self):
        ...
        self.client.force_login(self.user)
        self.assertEqual(
            self.client.get(reverse("abbreviation_tool:landing")).status_code, 404
        )
    ...
    def test_dictionary_search_uses_imported_jssdm_data(self):
        ...
        response = self.client.get(
            reverse("abbreviation_tool:dictionary"), {"q": "Situation Report"}
        )
        self.assertContains(response, "SITREP")

    def test_profiles_are_database_driven(self):
        self.assertTrue(
            AbbreviationProfile.objects.filter(name="General", active=True).exists()
        )
        self.assertTrue(
            AbbreviationProfile.objects.filter(name="Joint", active=True).exists()
        )

    def test_entry_normalization_and_unique_pair(self):
        first = AbbreviationEntry.objects.create(
            abbreviation=" TEST ", full_form="  Test   Entry "
        )
    ...
    def test_permission_groups_are_configured(self):
        self.assertEqual(
            Group.objects.get(name="DOCX Abbreviation Users").permissions.count(), 3
        )
        self.assertEqual(
            Group.objects.get(
