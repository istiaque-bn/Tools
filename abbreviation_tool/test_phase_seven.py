...
class FinalHardeningTests(TestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user("hardened-user")
        ...
        entry = AbbreviationEntry.objects.create(
            abbreviation="HQ", full_form="Headquarters", is_ambiguous=True
        )
    ...
    def test_analysis_limit_failure_deletes_files_via_view(self):
        # Patch analysis to exercise failure cleanup with this unmatched fixture.
        session = self.make_session()
        self.client.force_login(self.user)
        from django.core.exceptions import ValidationError

        with patch(
            "abbreviation_tool.views.analyse_session",
            side_effect=ValidationError("Safe limit reached."),
        ):
            response = self.client.post(
                reverse("abbreviation_tool:analyse", args=[session.id])
            )
    ...
    def test_force_case_sensitive_and_high_confidence_options(self):
        from abbreviation_tool.services.matching import (
            candidates_for,
            find_matches,
            POLICY_ALL,
        )
        ...
        candidate = candidates_for(
            self.profile, "abbreviate", force_case_sensitive=True
        )
        text = "headquarters"
        container = TextContainer(
            ...
            "paragraph",
            text,
            [CharacterLocation(0, i, b"") for i in range(len(text))],
        )
        self.assertEqual(
            find_matches(container, candidate, "abbreviate", POLICY_ALL), []
        )
    ...
    def test_simple_upload_processes_and_downloads(self):
        ...
        upload = SimpleUploadedFile(
            "simple.docx", output.getvalue(), content_type=DOCX_MIME
        )
        AbbreviationEntry.objects.filter(abbreviation="HQ").update(is_ambiguous=False)
        self.client.force_login(self.user)
        response = self.client.post(
            reverse("abbreviation_tool:landing"),
            {"operation_type": "abbreviate", "docx_file": upload},
        )
        self.assertEqual(response.status_code, 200)
        processed = b"".join(response.streaming_content)
        response.close()
    ...
    def test_two_way_text_converter(self):
        ...
        response = self.client.post(
            ...
            '{"operation":"abbreviate","text":"Report to Headquarters."}',
            content_type="application/json",
        )
        self.assertEqual(response.json()["result"], "Report to HQ.")
        response = self.client.post(
            reverse("abbreviation_tool:text_convert"),
            '{"operation":"deabbreviate","text":"Report to HQ."}',
            content_type="application/json",
        )
