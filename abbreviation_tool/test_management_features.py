...
class ManagementFeatureTests(TestCase):
    def setUp(self):
        self.admin = get_user_model().objects.create_user(
            "admin2", password="test", is_staff=True
        )
        self.client.force_login(self.admin)

    def test_staff_can_add_edit_and_import(self):
        response = self.client.post(
            reverse("abbreviation_tool:manage_dictionary"),
            {
                "entry-abbreviation": "WHO",
                "entry-full_form": "World Health Organization",
                "save_entry": "1",
            },
        )
        ...
        self.client.post(
            reverse("abbreviation_tool:edit_dictionary_entry", args=[entry.pk]),
            {
                "entry-abbreviation": "W.H.O.",
                "entry-full_form": "World Health Organization",
                "save_entry": "1",
            },
        )
        ...
        upload = SimpleUploadedFile(
            "entries.csv",
            b"abbreviation,full_form\nUN,United Nations\n",
            content_type="text/csv",
        )
        self.client.post(
            reverse("abbreviation_tool:manage_dictionary"),
            {"import-file": upload, "import_entries": "1"},
        )
        self.assertTrue(AbbreviationEntry.objects.filter(abbreviation="UN").exists())

    def test_feedback_is_stored(self):
        self.client.post(
            reverse("abbreviation_tool:feedback"),
            {
                "name": "Admin",
                "email": "admin@example.com",
                "message": "Please correct an entry.",
            },
        )
    ...
    def test_staff_can_delete_entry_and_sees_success(self):
        entry = AbbreviationEntry.objects.create(
            abbreviation="DEL", full_form="Delete Example"
        )
        response = self.client.post(
            reverse("abbreviation_tool:delete_dictionary_entry", args=[entry.pk]),
            follow=True,
        )
    ...
    def test_management_list_searches_abbreviation_and_full_form(self):
        AbbreviationEntry.objects.create(
            abbreviation="WHO", full_form="World Health Organization"
        )
        AbbreviationEntry.objects.create(abbreviation="UN", full_form="United Nations")
        response = self.client.get(
            reverse("abbreviation_tool:manage_dictionary"), {"q": "WHO"}
        )
        self.assertContains(response, "World Health Organization")
        self.assertEqual(
            list(
                response.context["search_results"].values_list(
        ...
        response = self.client.get(
            reverse("abbreviation_tool:manage_dictionary"), {"q": "Nations"}
        )
    ...
    def test_management_page_shows_total_entry_count(self):
        ...
        self.assertEqual(
            response.context["total_entries"], AbbreviationEntry.objects.count()
        )
    ...
    def test_staff_can_import_xlsx(self):
        ...
        self.assertTrue(
            AbbreviationEntry.objects.filter(
                abbreviation="EU", full_form="European Union"
            ).exists()
        )
    ...
    def test_staff_can_export_every_abbreviation_to_xlsx(self):
        ...
        workbook = load_workbook(
            BytesIO(b"".join(response.streaming_content)), read_only=True
        )
    ...
    def test_multiple_full_forms_are_kept_as_separate_ambiguous_meanings(self):
        AbbreviationEntry.objects.create(
            abbreviation="CO", full_form="Commanding Officer"
        )
        AbbreviationEntry.objects.create(abbreviation="CO", full_form="Company")
        response = self.client.get(
            reverse("abbreviation_tool:manage_dictionary"), {"q": "CO"}
        )
    ...
    def test_define_first_avoids_duplicate_parentheses(self):
        ...
        text = (
            "World Health Organization (WHO) works. World Health Organization reports."
        )
        locations = [CharacterLocation(0, i, b"") for i in range(len(text))]
        matches = find_matches(
            TextContainer("text", "p", "paragraph", text, locations),
            candidates_for(None, "abbreviate"),
            "abbreviate",
            POLICY_DEFINE_FIRST,
        )
    ...
    def test_text_converter_abbreviates_every_occurrence_without_definition(self):
        ...
        response = self.client.post(
            reverse("abbreviation_tool:text_convert"),
            '{"operation":"abbreviate","text":"World Health Organization and World Health Organization"}',
    ...
    def test_powerpoint_converter_preserves_presentation_and_defines_first_use(self):
        entry = AbbreviationEntry.objects.create(
            abbreviation="HQ", full_form="Headquarters"
        )
        ...
        response = self.client.post(
            reverse("abbreviation_tool:powerpoint_convert"),
            {"operation_type": "abbreviate", "presentation_file": upload},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_powerpoint_abbreviates_all_full_forms_even_when_abbreviation_is_ambiguous(
        self,
    ):
        general = AbbreviationProfile.objects.get(name="General")
        officer = AbbreviationEntry.objects.create(
            abbreviation="CO", full_form="Commanding Officer", is_ambiguous=True
        )
        company = AbbreviationEntry.objects.create(
            abbreviation="CO", full_form="Company", is_ambiguous=True
        )
        ...
        box.text_frame.paragraphs[
            0
        ].text = "Commanding Officer met the Commanding Officer at the Company."
        content = BytesIO()
        presentation.save(content)
        upload = SimpleUploadedFile(
            "ambiguous.pptx",
        ...
        converted = Presentation(BytesIO(b"".join(response.streaming_content)))
        self.assertEqual(
            converted.slides[0].shapes[0].text,
            "Commanding Officer (CO) met the CO at the Company (CO).",
        )

    def test_legacy_ppt_receives_safe_conversion_message(self):
        upload = SimpleUploadedFile(
            "legacy.ppt", b"legacy", content_type="application/vnd.ms-powerpoint"
        )
        response = self.client.post(
            reverse("abbreviation_tool:powerpoint_convert"),
            {"operation_type": "abbreviate", "presentation_file": upload},
    ...
    def test_longer_full_form_is_matched_before_nested_shorter_form(self):
        AbbreviationEntry.objects.create(
            abbreviation="BAF", full_form="Bangladesh Armed Force"
        )
        AbbreviationEntry.objects.get_or_create(
            abbreviation="BD", full_form="Bangladesh"
        )
        ...
        self.assertEqual(
            [match.original for match in matches],
            ["Bangladesh Armed Force", "Bangladesh"],
        )
        self.assertEqual(
            [match.proposed for match in matches],
            ["Bangladesh Armed Force (BAF)", "Bangladesh (BD)"],
        )
    ...
    def test_managed_long_phrase_is_available_in_general_profile(self):
        self.client.post(
            ...
            {
                "save_entry": "1",
                "entry-abbreviation": "BDAF",
                "entry-full_form": "Bangladesh Armed Forces",
            },
        )
        general = AbbreviationProfile.objects.get(name="General")
        ...
        matches = find_matches(
            TextContainer("text", "p", "paragraph", text, locations),
            candidates_for(general, "abbreviate"),
            "abbreviate",
            POLICY_DEFINE_FIRST,
        )
        self.assertEqual(len(matches), 1)
