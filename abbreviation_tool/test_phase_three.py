...
from .models import AbbreviationEntry, AbbreviationProfile, AbbreviationVariant, DocumentProcessingSession
from .services.analysis import analyse_session
from .services.matching import POLICY_ALL, POLICY_DEFINE_FIRST, POLICY_KEEP_FIRST, candidates_for, find_matches
from .services.ooxml import CharacterLocation, TextContainer, document_containers
from .storage import save_original
from .test_phase_two import CONTENT_TYPES, DOCX_MIME, RELS, docx_upload
...
class MatchingEngineTests(TestCase):
    ...
    def entry(self, abbreviation, full_form, **kwargs):
        entry = AbbreviationEntry.objects.create(abbreviation=abbreviation, full_form=full_form, **kwargs)
    ...
    def test_longest_match_first_and_whole_words(self):
        ...
        matches = find_matches(container("Bangladesh Armed Forces and Armed Forces, not Armed ForcesCommander."), candidates_for(self.profile, "abbreviate"), "abbreviate", POLICY_ALL)
        self.assertEqual([(m.entry, m.original) for m in matches], [(long, "Bangladesh Armed Forces"), (short, "Armed Forces")])
    ...
    def test_case_punctuation_and_variant(self):
        ...
        AbbreviationVariant.objects.create(entry=entry, variant="Commanding Officers", variant_type="plural")
        matches = find_matches(container("Commanding Officer, commanding officer; Commanding Officers."), candidates_for(self.profile, "abbreviate"), "abbreviate", POLICY_ALL)
        self.assertEqual([m.original for m in matches], ["Commanding Officer", "Commanding Officers"])
    ...
    def test_urls_emails_and_existing_definitions_are_excluded(self):
        ...
        matches = find_matches(text, candidates_for(self.profile, "deabbreviate"), "deabbreviate", POLICY_ALL)
        self.assertEqual([m.original for m in matches], ["HQ"])

    def test_ambiguous_expansion_is_flagged(self):
        first = self.entry("CO", "Commanding Officer", is_ambiguous=True, is_preferred=True)
        self.entry("CO", "Company", is_ambiguous=True)
        matches = find_matches(container("CO"), candidates_for(self.profile, "deabbreviate"), "deabbreviate", POLICY_ALL)
    ...
    def test_cross_run_mixed_format_detection(self):
        ...
        matches = find_matches(container(text, styles), candidates_for(self.profile, "abbreviate"), "abbreviate", POLICY_ALL)
...
class AnalysisIntegrationTests(TestCase):
    def setUp(self):
        ...
        self.profile = AbbreviationProfile.objects.get(name="General")
        entry = AbbreviationEntry.objects.create(abbreviation="BAF", full_form="Bangladesh Armed Forces")
        entry.profiles.add(self.profile)
    ...
    def cross_run_upload(self):
        ...
        return SimpleUploadedFile("cross-run.docx", output.getvalue(), content_type=DOCX_MIME)

    def test_ooxml_mapping_and_persisted_analysis(self):
        upload = self.cross_run_upload()
        session = DocumentProcessingSession.objects.create(user=self.user, original_filename="test.docx", operation_type="abbreviate", profile=self.profile, file_size=upload.size, expires_at=timezone.now() + timedelta(minutes=30))
        save_original(session, upload)
        containers = document_containers(self.temp.name + "/" + str(session.id) + "/original.docx")
