...
class DictionarySearchForm(forms.Form):
    ...
    status = forms.ChoiceField(
        required=False,
        choices=(("", "All statuses"), *AbbreviationEntry.Status.choices),
    )
...
class AbbreviationEntryForm(forms.ModelForm):
    class Meta:
        ...
        help_texts = {
            "full_form": "For multiple meanings, add each full form as a separate entry using the same abbreviation."
        }
    ...
    def clean_full_form(self):
        ...
        if " / " in full_form:
            raise forms.ValidationError(
                "Add each meaning as a separate entry instead of separating full forms with '/'."
            )
        return full_form
    ...
    def clean(self):
        ...
        duplicate = AbbreviationEntry.objects.filter(
            normalized_abbreviation=short, normalized_full_form=full
        )
        if self.instance.pk:
            duplicate = duplicate.exclude(pk=self.instance.pk)
        if short and full and duplicate.exists():
            raise forms.ValidationError(
                "This abbreviation and full-form pair already exists."
            )
        return cleaned
...
class DictionaryImportForm(forms.Form):
    file = forms.FileField(
        help_text="CSV or XLSX with abbreviation and full_form columns (maximum 2 MB)."
    )
...
class DocumentUploadForm(forms.Form):
    GLOSSARY_CHOICES = (
        ...
        ("separate", "Download separately"),
        ("insert_end", "Insert at end of document"),
        ("bookmark", "Insert at configured bookmark"),
    )
    operation_type = forms.ChoiceField(
        choices=DocumentProcessingSession.Operation.choices, widget=forms.RadioSelect
    )
    profile = forms.ModelChoiceField(queryset=AbbreviationProfile.objects.none())
    pdf_file = forms.FileField(label="DOCX document")
    replacement_policy = forms.ChoiceField(
        choices=DocumentProcessingSession.Policy.choices,
        initial=DocumentProcessingSession.Policy.DEFINE_FIRST,
    )
    ...
    glossary_mode = forms.ChoiceField(
        choices=GLOSSARY_CHOICES, initial="none", required=False
    )
    glossary_bookmark = forms.CharField(required=False, max_length=100)

    def clean(self):
        cleaned = super().clean()
        if cleaned.get("glossary_mode") == "bookmark" and not cleaned.get(
            "glossary_bookmark"
        ):
            self.add_error(
                "glossary_bookmark",
                "Enter the Word bookmark where the glossary should be inserted.",
            )
        return cleaned

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["profile"].queryset = AbbreviationProfile.objects.filter(
            active=True
        )
...
class QuickProcessForm(forms.Form):
    operation_type = forms.ChoiceField(
        choices=DocumentProcessingSession.Operation.choices,
        widget=forms.RadioSelect,
        initial=DocumentProcessingSession.Operation.ABBREVIATE,
    )
...
class PowerPointProcessForm(forms.Form):
    ...
    def clean_presentation_file(self):
        ...
        if name.endswith(".ppt"):
            raise forms.ValidationError(
                "Legacy .ppt files cannot be safely edited. Open the file in PowerPoint or LibreOffice and save it as .pptx first."
            )
