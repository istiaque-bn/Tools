...
def profile_entries(profile):
    ...
    return entries.select_related("category").prefetch_related(
        "variants", "preferred_in_profiles"
    )
...
def candidates_for(profile, operation, force_case_sensitive=False):
    ...
    for entry in profile_entries(profile):
        ...
        candidates.append(
            Candidate(
                entry, phrase, False, force_case_sensitive or entry.case_sensitive
            )
        )
        variant_type = (
            AbbreviationVariant.VariantType.FULL_FORM
            if operation == DocumentProcessingSession.Operation.ABBREVIATE
            else AbbreviationVariant.VariantType.ABBREVIATION
        )
        for variant in entry.variants.all():
            if (
                variant.status == AbbreviationEntry.Status.ACTIVE
                and variant.variant_type
                in {
                    variant_type,
                    AbbreviationVariant.VariantType.PLURAL,
                    AbbreviationVariant.VariantType.POSSESSIVE,
                    AbbreviationVariant.VariantType.HYPHENATED,
                    AbbreviationVariant.VariantType.SPELLING,
                }
            ):
                candidates.append(
                    Candidate(
                        ...
                        variant.variant,
                        True,
                        force_case_sensitive or variant.case_sensitive,
                    )
                )
    return sorted(
        candidates,
...
def existing_definition_ranges(text):
    return [
        (match.start(), match.end())
        for match in re.finditer(r"\b[^()]{2,100}\s+\([A-Z][A-Z0-9&/ .-]{1,30}\)", text)
    ]


def find_matches(
    container, candidates, operation, policy=POLICY_DEFINE_FIRST, seen=None
):
    seen = seen if seen is not None else set()
    occupied = []
    results = []
    excluded = excluded_ranges(container.text) + existing_definition_ranges(
        container.text
    )
    meanings = {}
    for candidate in candidates:
        meanings.setdefault(candidate.entry.normalized_abbreviation, set()).add(
            candidate.entry.normalized_full_form
        )
    for candidate in candidates:
        for found in phrase_pattern(
            candidate.phrase, candidate.case_sensitive
        ).finditer(container.text):
            ...
            ambiguous = (
                len(meanings.get(entry.normalized_abbreviation, ())) > 1
                or entry.is_ambiguous
            )
            if operation == DocumentProcessingSession.Operation.ABBREVIATE:
                ...
                already_defined = any(
                    match.end() <= start
                    for match in definition.finditer(container.text)
                )
                ...
                if policy == POLICY_KEEP_FIRST and first:
                    ...
                    continue
                proposed = (
                    f"{found.group(0)} ({entry.abbreviation})"
                    if policy == POLICY_DEFINE_FIRST and first
                    else entry.abbreviation
                )
                seen.add(identity)
            else:
            ...
            confidence = (
                100.0
                if not candidate.is_variant and not ambiguous
                else 90.0
                if candidate.is_variant and not ambiguous
                else 60.0
            )
            results.append(
                Match(
                    ...
                    container.mixed_format(start, end),
                )
            )
            occupied.append((start, end))
