...
def build_preview(session):
    suggestions = list(
        session.suggestions.order_by("container_identifier", "start_offset")
    )
    by_container = {}
    ...
    for container in document_containers(
        ...
        for suggestion in by_container.get(container.identifier, []):
            if suggestion.start_offset > cursor:
                segments.append(
                    PreviewSegment(container.text[cursor : suggestion.start_offset])
                )
            segments.append(
                PreviewSegment(
                    ...
                    str(suggestion.id),
                    "ambiguous"
                    if suggestion.ambiguity_status == "ambiguous"
                    and suggestion.review_status == "pending"
                    else suggestion.review_status,
                )
            )
        ...
        preview.append(
            PreviewContainer(container.identifier, container.container_type, segments)
        )
    return preview
