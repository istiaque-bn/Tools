...
def _visible_mapping(paragraph):
    ...
    for node in paragraph.iter():
        if node.tag == f"{W}t" and node.text:
            ...
            text.append("\t"); mapping.append((None, 0))
...
def replace_range(paragraph, start, end, replacement, expected):
    ...
    if start < 0 or end > len(mapping) or start >= end or visible[start:end] != expected:
        raise ValidationError("The document structure no longer matches its analysed suggestion offsets.")
    selected = mapping[start:end]
    if any(node is None for node, _ in selected):
        raise ValidationError("A replacement crosses an unsupported line break or tab boundary.")
    start_node, start_offset = selected[0]
    end_node, end_offset = selected[-1]
    if start_node is end_node:
        original = start_node.text or ""
        start_node.text = original[:start_offset] + replacement + original[end_offset + 1:]
...
def apply_part_suggestions(xml_bytes, suggestions):
    ...
    for suggestion in suggestions:
        try:
            ...
            raise ValidationError("A suggestion has an invalid paragraph location.") from exc
    ...
    for index, paragraph_suggestions in grouped.items():
        ...
        for suggestion in sorted(paragraph_suggestions, key=lambda item: item.start_offset, reverse=True):
            replacement = suggestion.user_modified_text or suggestion.proposed_text
            replace_range(paragraphs[index], suggestion.start_offset, suggestion.end_offset, replacement, suggestion.original_text)
    return etree.tostring(root, xml_declaration=True, encoding="UTF-8", standalone=None)


def glossary_rows(session):
    rows = {}
    suggestions = session.suggestions.filter(review_status__in=(ProcessingSuggestion.ReviewStatus.ACCEPTED, ProcessingSuggestion.ReviewStatus.EDITED)).select_related("abbreviation_entry", "selected_meaning")
    for suggestion in suggestions:
        entry = suggestion.selected_meaning or suggestion.abbreviation_entry
        rows[(entry.abbreviation.casefold(), entry.full_form.casefold())] = (entry.abbreviation, entry.full_form)
...
def insert_glossary(xml_bytes, rows, bookmark=""):
    ...
    if bookmark:
        marker = root.xpath(".//w:bookmarkStart[@w:name=$name]", namespaces={"w": W[1:-1]}, name=bookmark)
        if not marker:
        ...
        if paragraph is None or paragraph.getparent() is not body:
            raise ValidationError("The glossary bookmark is not in a supported body paragraph.")
...
def _write_package(original, destination, changed_parts):
    ...
    try:
        ...
        with zipfile.ZipFile(original) as source, zipfile.ZipFile(temporary, "w") as output:
        ...
        with zipfile.ZipFile(temporary) as package:
            if "word/document.xml" not in package.namelist():
                raise ValidationError("Generated DOCX is missing its main document part.")
            etree.fromstring(package.read("word/document.xml"), etree.XMLParser(resolve_entities=False, no_network=True))
            bad = package.testzip()
            if bad:
                raise ValidationError(f"Generated DOCX contains a corrupted package entry: {bad}")
...
def generate_session(session_id, user):
    with transaction.atomic():
        ...
        if session.suggestions.filter(ambiguity_status="ambiguous", review_status="pending").exists():
            raise ValidationError("Resolve or reject every ambiguous suggestion before generating the DOCX.")
        suggestions = list(session.suggestions.filter(review_status__in=(ProcessingSuggestion.ReviewStatus.ACCEPTED, ProcessingSuggestion.ReviewStatus.EDITED)).order_by("container_identifier", "-start_offset"))
        if not suggestions:
            raise ValidationError("Accept or edit at least one suggestion before generating the DOCX.")
        ...
        with zipfile.ZipFile(original) as package:
            for part, part_suggestions in grouped.items():
                if part not in package.namelist():
                    raise ValidationError("A suggestion points to a missing OOXML part.")
                changed_parts[part] = apply_part_suggestions(package.read(part), part_suggestions)
            mode = session.processing_options.get("glossary_mode", "none")
            if mode in {"insert_end", "bookmark"}:
                main_xml = changed_parts.get("word/document.xml", package.read("word/document.xml"))
                changed_parts["word/document.xml"] = insert_glossary(main_xml, glossary_rows(session), session.processing_options.get("glossary_bookmark", "") if mode == "bookmark" else "")
