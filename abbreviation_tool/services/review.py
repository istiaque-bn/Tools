...
def _restore(snapshot):
    ...
    suggestions = {
        str(item.id): item for item in ProcessingSuggestion.objects.filter(id__in=ids)
    }
    changed = []
    ...
    ProcessingSuggestion.objects.bulk_update(
        changed, ("review_status", "user_modified_text", "selected_meaning")
    )


def _update_counts(session):
    session.accepted_count = session.suggestions.filter(
        review_status__in=("accepted", "edited")
    ).count()
    session.rejected_count = session.suggestions.filter(
        review_status="rejected"
    ).count()
    session.save(
        update_fields=("accepted_count", "rejected_count", "processing_options")
    )


@transaction.atomic
def decide(
    session_id, user, suggestion_id, action, replacement="", selected_meaning_id=None
):
    ...
    if action == "accept":
        if suggestion.ambiguity_status == "ambiguous" and not selected_meaning_id:
            raise ValidationError(
                "Select a meaning before accepting an ambiguous abbreviation."
            )
    ...
    if selected_meaning_id:
        selected = AbbreviationEntry.objects.get(
            id=selected_meaning_id,
...
def bulk_decide(session_id, user, action, high_confidence=False, suggestion_ids=None):
    ...
    if action == "accept":
        queryset.filter(ambiguity_status="unambiguous").update(
            review_status="accepted", user_modified_text=""
        )
    elif action == "reject":
        queryset.update(review_status="rejected", user_modified_text="")
    elif action == "reset":
        queryset.update(
            review_status="pending", user_modified_text="", selected_meaning=None
        )
    else:
...
def history_action(session_id, user, direction):
    ...
    options = _history(session)
    source, target = (
        ("review_undo", "review_redo")
        if direction == "undo"
        else ("review_redo", "review_undo")
    )
