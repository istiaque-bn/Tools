...
def analyse_session(session, policy="define_first", include_tables=True):
    ...
    try:
        ...
        candidates = candidates_for(
            ...
            session.processing_options.get("case_sensitive", False),
        )
        containers = document_containers(
            session_directory(session.id) / ORIGINAL_NAME,
            include_tables=include_tables,
            include_headers_footers=session.processing_options.get(
                "include_headers_footers", False
            ),
            include_footnotes_endnotes=session.processing_options.get(
        ...
        for container in containers:
            for match in find_matches(
                container, candidates, session.operation_type, policy=policy, seen=seen
            ):
                if (
                    monotonic() - started
                    > settings.DOCX_ABBREVIATION_ANALYSIS_TIMEOUT_SECONDS
                ):
                    raise ValidationError(
                        "Document analysis exceeded the configured time limit."
                    )
                if (
                    session.processing_options.get("high_confidence_only")
                    and match.confidence < 90
                ):
                    continue
                if len(suggestions) >= settings.DOCX_ABBREVIATION_MAX_SUGGESTIONS:
                    raise ValidationError(
                        "The document produced more suggestions than the configured safe limit."
                    )
                suggestions.append(
        ...
        session.ambiguous_count = sum(
            item.ambiguity_status == "ambiguous" for item in suggestions
        )
