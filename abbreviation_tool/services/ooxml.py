...
class TextContainer:
    ...
    def mixed_format(self, start, end):
        styles = {
            location.style_signature for location in self.character_map[start:end]
        }
        return len(styles) > 1
...
def paragraph_container(paragraph, part, index, container_type="paragraph"):
    ...
    return TextContainer(
        part, f"{part}:p{index}", container_type, "".join(visible), mapping
    )


def supported_parts(
    package, include_headers_footers=False, include_footnotes_endnotes=False
):
    names = set(package.namelist())
    parts = [("word/document.xml", "body")]
    if include_headers_footers:
        parts.extend(
            (name, "header" if "/header" in name else "footer")
            for name in sorted(names)
            if name.startswith(("word/header", "word/footer")) and name.endswith(".xml")
        )
    if include_footnotes_endnotes:
        parts.extend(
            (name, "footnote" if name.endswith("footnotes.xml") else "endnote")
...
def document_containers(
    ...
    with zipfile.ZipFile(docx_path) as package:
        roots = [
            (part, part_type, ElementTree.fromstring(package.read(part)))
            for part, part_type in supported_parts(
                package, include_headers_footers, include_footnotes_endnotes
            )
        ]
    containers = []
    for part, part_type, root in roots:
        for index, paragraph in enumerate(root.iter(f"{W}p")):
            in_table = any(
                ancestor.tag == f"{W}tc" for ancestor in _ancestors(root, paragraph)
            )
            ...
            container_type = (
                "table"
                if in_table
                else ("paragraph" if part_type == "body" else part_type)
            )
